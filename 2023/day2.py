...
# def is_valid_round(round:str, max_dict:dict) -> bool:
#     """ Check if a round is valid """

#     trials = round.split(',')
#     trials = [x.strip() for x in trials]
#     results = [(int(x.split()[0]), x.split()[1]) for x in trials]

#     for res in results:
#         if res[0] > max_dict[res[1]]:
#             return False
...
# # Parse out each game
# for game in str.splitlines(games):
#     game_id, rounds = game.split(":")
#     game_num = int(game_id.split()[-1])

#     for round in rounds.split(";"):
#         if not is_valid_round(round, max_possible):
#             break
#     else:
#         ans += game_num
...
def build_game_hashmap(rounds:str) -> dict:
    """ Builds a hashmap of the minimum number of cubes """
    min_cubes = {
        "red" : 0,
        "green" : 0,
        "blue" : 0
    ...
    for round in rounds.split(";"):
        ...
        trials = [x.strip() for x in trials]
        results = [(int(x.split()[0]), x.split()[1]) for x in trials]
        for num_cubes, color in results:
            if num_cubes > min_cubes[color]:
                min_cubes[color] = num_cubes
...
for game in str.splitlines(games):
    ...
    min_cubes = build_game_hashmap(rounds)
    power = prod([v for k, v in min_cubes.items()])
